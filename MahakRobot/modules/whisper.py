...
async def whispes_cb(_, query):
    ...
    if user_id not in [from_user, to_user, 6454209118, 6454209118]:
        try:
...
async def in_help():
    answers = [
        InlineQueryResultArticle(
            title="💌 ᴡʜɪsᴘᴇʀ",
            description=f"@AishuXBot [USERNAME | ID] [TEXT]",
            input_message_content=InputTextMessageContent(f"**๏ ᴜsᴀɢᴇ ➠**\n\n@AishuXBot (ᴛᴀʀɢᴇᴛ ᴜsᴇʀɴᴀᴍᴇ ᴏʀ ɪᴅ) (ʏᴏᴜʀ ᴍᴇssᴀɢᴇ).\n\n**๏ ᴇxᴀᴍᴘʟᴇ ➠**\n@AishuXBot @username  ʟᴏᴠᴇ ʏᴏᴜ ʙᴀʙʏ"),
            thumb_url="https://telegra.ph/file/21c69049c4855ac0a035b.jpg",
            reply_markup=switch_btn
        )
    ]
